...
######################################
    ...
    ## Gm(t) = sum across dictionary entries of e^(((T1_gm-T1)/sigmaT1_gm)**2 + ((T2_gm-T2)/sigmaT2_gm)**2)
    ...
    # Added -1 to the exponential term to have peak at T1/T2 = T1_gm/T2_gm
    ...
    ## Use a pseudoinversion to calculate the alpha/beta/gamma weights for all entries in the original dictionary, as if they were image voxels. 
    ...
    basis = torch.tensor(np.stack([WM, GM, CSF]))
